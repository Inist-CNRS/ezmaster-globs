...
export GITLAB_HTTP_BASEURL="${GITLAB_HTTP_BASEURL:="$(jq -r -M '.GITLAB_HTTP_BASEURL' /config.json | grep -v null)"}"
export GITLAB_SSH_BASEURL="${GITLAB_SSH_BASEURL:="$(jq -r -M '.GITLAB_SSH_BASEURL' /config.json | grep -v null)"}"
...
export GITLAB_GROUP_PREFIX="${GITLAB_GROUP_PREFIX:="$(jq -r -M '.GITLAB_GROUP_PREFIX' /config.json | grep -v null)"}"
